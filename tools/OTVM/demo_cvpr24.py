...
from dataset import EvalCVPR24Dataset, VideoMatting108_Test, Demo_MultiInstance
from helpers import *
...
def parse_args():
    ...
    cfg.DATASET.PATH = args.data_dir
...
def main(cfg, args, GPU):
    ...
    alpha_outdir = output_dir #os.path.join(output_dir, 'test', outdir_tail)
    ...
    load_ckpt = "/sensei-fs/users/chuongh/OTVM/train_log/checkpoint/s4_OTVM/checkpoint_5.pth"
    ...
    if args.demo:
        valid_dataset = Demo_MultiInstance(data_root=cfg.DATASET.PATH)
    else:
...
def eval(args, cfg, valid_dataset, model, alpha_outdir, viz_outdir_img, viz_outdir_vid, VIZ):
    ...
    for i_iter, (data_name, data_root, FG, BG, a, tri, seq_name) in enumerate(valid_dataset):
        ...
        num_frames = 1
        eval_sequence = EvalCVPR24Dataset(
            data_name=data_name,
            data_root=data_root,
            FG=FG,
            BG=BG,
        ...
        save_path = os.path.join(alpha_outdir, seq_name[:-3])
        ...
        for i_seq, dp in enumerate(iterations):
            ...
            def write_result_images(alpha_pred_img, path, VIZ, frame_path, vis_out, i_seq):
                ...
                # Save trimap as binary mask
                tri_pred = vis_out[1][0,0].argmax(dim=0).cpu().numpy()
                path = path.replace("xmem_finetuned", "mask")
                os.makedirs(os.path.dirname(path), exist_ok=True)
                cv2.imwrite(path, (tri_pred > 0).astype(np.uint8) * 255)
            
            alpha_path = os.path.join(save_path, os.path.splitext(filename)[0], seq_name[-2:] + ".png")
            os.makedirs(os.path.dirname(alpha_path), exist_ok=True)

            # TODO: Log trimap prediction as well
