...
def generator_tensor_dict(image_path, mask_paths, min_side=576):
    # read images
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image-pattern', type=str, default='datasets/images', help="input image dir")
    ...
    parser.add_argument('--min-side', type=int, default=576, help="maximum side length")
    ...
    # inference
    model = model.eval()
    print("No.parameters: ", sum([p.numel() for p in model.parameters()]))
    ...
    image_list = list(glob.glob(args.image_pattern))
    ...
    # image_list = ['/mnt/localssd/HIM2K/images/natural/google_middle_62604586fc08499ca4d346900c338bc8.jpg']
    # for n_inst in range(10):

    tracked_time = collections.defaultdict(list)
    tracked_memory = collections.defaultdict(list)
    random_state = np.random.RandomState(2023)
    # for j in tqdm.tqdm(range(100)):
    for image_path in image_list:
        mask_paths = list(glob.glob(os.path.join(args.mask_dir, os.path.basename(image_path).replace('.jpg', '')) + '/*.png'))
        mask_paths = sorted(mask_paths)
        # mask_paths = [image_path.replace(args.image_dir, args.mask_dir)]
        ...
        if len(mask_paths) == 0:
            continue
        # mask_paths = random_state.choice(mask_paths, size=n_inst+1, replace=False)
        image_dict = generator_tensor_dict(image_path, mask_paths, min_side=args.min_side)
        ...
        # dst_sub_dir = os.path.join(args.output, os.path.relpath(os.path.dirname(image_path), args.image_dir))
        # os.makedirs(dst_sub_dir, exist_ok=True)
        # cv2.imwrite(image_path[:-4]+'_pred.png', alpha_pred)
        # cv2.imwrite(os.path.join(dst_sub_dir, os.path.basename(image_path)), alpha_pred[:, :, 0])
        output_dir = os.path.join(args.output, os.path.basename(image_path).replace('.jpg', ''))
        os.makedirs(output_dir, exist_ok=True)
        for inst_i in range(alpha_pred.shape[2]):
            mask_name = os.path.basename(mask_paths[inst_i])
            cv2.imwrite(os.path.join(output_dir, mask_name), alpha_pred[:,:,inst_i])
        # import pdb; pdb.set_trace()
    ...
    #     print("Mem: {} - {}".format(k, sum(tracked_memory[k])/ len(tracked_memory[k]) / 1024.0))
    #     print("Time: {} - {}".format(k, sum(tracked_time[k])/ len(tracked_time[k])*1000))
    print(f"Average batch time: {batch_time.avg:.4f}")
    # import pickle
    # pickle.dump(tracked_time, open('tracked_time_natural.pkl', 'wb'))
    # pickle.dump(tracked_memory, open('tracked_memory_natural.pkl', 'wb'))
