...
from vm2m.network.module.mask_matte_dyn_atten import MaskMatteDynAttenHead

class ResShortCut_DynAtten_Dec(nn.Module):
    def __init__(self, block, layers, norm_layer=None, large_kernel=False, 
        ...
        super(ResShortCut_DynAtten_Dec, self).__init__()
        ...
        ## OS1
        self.refine_OS1 = MaskMatteDynAttenHead(
            input_dim=32,
        ...
        ## OS4
        self.refine_OS4 = MaskMatteDynAttenHead(
            input_dim=64,
        ...
        ## 1/8 scale
        self.refine_OS8 = MaskMatteDynAttenHead(
            input_dim=128,
    ...
    def forward(self, x, mid_fea, b, n_f, n_i, masks, **kwargs):
        ...
        os4_masks = self.compute_unknown(x_os8, k_size=30)
        os4_masks = os4_masks.reshape(b, n_f, n_i, *os4_masks.shape[-2:])
        x_os4, x = self.refine_OS4(x, os4_masks)
        ...
        os1_masks = self.compute_unknown(x_os4, k_size=15)
        os1_masks = os1_masks.reshape(b, n_f, n_i, *os1_masks.shape[-2:])
...
def res_shortcut_dyn_attention_decoder_22(**kwargs):
    return ResShortCut_DynAtten_Dec(BasicBlock, [2, 3, 3, 2], **kwargs)