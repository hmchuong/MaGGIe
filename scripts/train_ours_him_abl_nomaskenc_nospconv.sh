...
torchrun --nproc_per_node=$RUNAI_NUM_OF_GPUS --nnodes=$WORLD_SIZE --node_rank=$RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT \
            tools/main_ddp.py --config $CONFIG --precision 16 name ours_${WORLD_SIZE}_gpus-${RUNAI_NUM_OF_GPUS}_no-mask_enc_no-spconv-dec_0830 output_dir output/HIM/ablation_study \
