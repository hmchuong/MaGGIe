...
torchrun --nproc_per_node=$RUNAI_NUM_OF_GPUS --nnodes=$WORLD_SIZE --node_rank=$RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT tools/main_ddp.py --config $CONFIG --precision 16 name ours_short-576-512x512_bs12_nnodes-${WORLD_SIZE}_gpus-${RUNAI_NUM_OF_GPUS}_30k_adamw_1.5e-4_scratch_0830