...
do
    ...
    do
    python -m tools.main --dist --gpus 4 --dist-url tcp://127.0.0.1:$2 --config output/HIM/ablation_loss/$1/config.yaml --eval-only \
                                                    name $1 \
                                                    model.weights output/HIM/ablation_loss/$1/best_model.pth \
