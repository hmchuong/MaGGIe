...
class FPN(nn.Module):
    def __init__(self, inplanes):
        ...
        atten_planes = 256
        breakdown_planes = 256

        # From os32 to os16
        self.conv1_init32 = conv1x1(inplanes['os32'], atten_planes) # convert os32-512 to os32-256
        self.layer1 = self._make_layer(BasicBlock, atten_planes, atten_planes, 2) # convert os32-512 to os16-256
        self.conv1_init16 = SpectralNorm(conv1x1(inplanes['os16'], atten_planes)) # convert os16-128 to os8-256
        self.smooth1_atten = conv1x1(atten_planes, atten_planes) # smooth os16-256
        
        # From os16 to os8
        self.layer2 = self._make_layer(BasicBlock, atten_planes, atten_planes, 2) # convert os16-256 to os8-256
        self.conv2_init8 = SpectralNorm(conv1x1(inplanes['os8'], atten_planes)) # convert os8-128 to os8-256
        self.smooth2_atten = conv1x1(atten_planes, atten_planes) # smooth os8-256
        self.smooth2_breakdown = conv1x1(atten_planes, breakdown_planes) # smooth os8-256 for breakdown

        # # From os8 to os4
        # self.layer3 = self._make_layer(BasicBlock, atten_planes, atten_planes, 2) # convert os8-256 to os4-256
        # self.conv3_init = SpectralNorm(conv1x1(inplanes['os4'], atten_planes)) # convert os4-64 to os4-256
        ...
        # # From os4 to os2
        # self.layer4 = self._make_layer(BasicBlock, atten_planes, inplanes['os2'], 2) # convert os4-256 to os2-256

        # # From os2 to os1
        # self.layer5 = self._make_layer(BasicBlock, inplanes['os2'], inplanes['os1'], 2) # convert os2-32 to os1-32
        # self.smooth5_breakdown = SpectralNorm(conv1x1(inplanes['os1'], breakdown_planes)) # smooth os1-32 for breakdown
    ...
    def forward(self, feats):
        ...
        '''
        # _, _, _, x3, x4, x5 = feats  # 1, 1/2, 1/4, 1/8, 1/16, 1/32
        x5 = feats['os32']
        x4 = feats['os16']
        x3 = feats['os8']
        attention_outputs = [] # 1/32, 1/16, 1/8
        mask_outputs = [] # 1/8

        x = self.conv1_init32(x5)
        attention_outputs += [x]

        x = self.layer1(x) + self.conv1_init16(x4) # 1/ 16: 256
        attention_outputs += [self.smooth1_atten(x)]

        x = self.layer2(x) + self.conv2_init8(x3) # 1/8: 256
