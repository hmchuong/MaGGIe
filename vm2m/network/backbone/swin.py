...
class SwinShortcut(nn.Module):
    def __init__(self, swin_backbone, in_features, out_features):
        super().__init__()
    ...
    def forward(self, x):
        ...
        fea3 = self.shortcut[2](feats['stage2'])
        fea4 = self.shortcut[3](feats['stage3'])
        fea5 = self.shortcut[4](feats['stage4'])
        out = self.shortcut[5](feats['stage5'])
        return out, {'shortcut': (fea1, fea2, fea3, fea4, fea5)}

def build_swin_backbone(patch_size, embed_dim, depths, num_heads, window_size, mlp_ratio, drop_path_rate, ape, freeze_at, out_features, num_mask=0, **kwargs):
    """
...
def build_swin_t(num_mask=0, **kawrgs):
    return build_swin_backbone(patch_size=4, 
                               embed_dim=96, 
                               depths=[2, 2, 6, 2], 
                               num_heads=[3, 6, 12, 24], 
...
def swin_shortcut_t(num_mask=0, pretrained=True, **kawrgs):
