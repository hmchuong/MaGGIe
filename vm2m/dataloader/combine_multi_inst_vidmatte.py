...
class CombineMultiInstVideoMatte(Dataset):
    def __init__(self, root_dir, split, clip_length, overlap=2, padding_inst=10, is_train=False, short_size=576, 
                    ...
                    max_step_size=5, random_seed=2023, ratio=[1, 1], pha_dir='pha', weight_mask_dir='', **kwargs):
        super().__init__()
        self.ratio = ratio
        ...
        # Pseudo-labels
        self.vipseg = MultiInstVidDataset(os.path.join(root_dir, "VIPSeg"), "filter_human", clip_length, overlap, padding_inst, is_train, short_size,
                                                crop, flip_p, bin_alpha_max_k,
    ...
    def __getitem__(self, idx):
        ...
        if data_i < self.ratio[0]:
            out = self.vm240k_syn[idx % len(self.vm240k_syn)]
            out['is_syn'] = True
        else:
            out = self.vipseg[idx % len(self.vipseg)]
            out['is_syn'] = False
        return out
