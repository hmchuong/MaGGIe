...
def build_dataset(cfg, is_train=True, random_seed=0):
    if cfg.name in ["VideoMatte240K", "polarized_matting"]:
        ...
        if is_train:
            ...
            dataset = HIMDataset(root_dir=cfg.root_dir, split=cfg.split, max_inst=cfg.max_inst, padding_inst=cfg.padding_inst, short_size=cfg.short_size, crop=cfg.crop, random_seed=random_seed, is_train=is_train, flip_p=cfg.flip_prob)
        else:
