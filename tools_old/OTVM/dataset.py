...
def apply_transforms_cv(xs, M):
    ...
    """
    dsize = (np.int64(xs[0].shape[1]), np.int64(xs[0].shape[0]))
...
class DIM_Train(torch.utils.data.Dataset):
    ...
    def random_crop(self, frame, mask, size, rnd):
        ...
        ## random transformations that both to be applied.
        min_scale = np.maximum(size[0]/np.float32(frame.shape[0]), size[1]/np.float32(frame.shape[1]))

        for t in range(10):
            scale = np.maximum(rnd.choice([1., 1./1.5, 1./2.]), min_scale+0.01)
            dsize = (np.int64(frame.shape[1]*scale), np.int64(frame.shape[0]*scale))
...
class EvalCVPR24Dataset(torch.utils.data.Dataset):
    def __init__(self,data_name, data_root, FG, BG, a, tri_gt,
                 trimap=None,
                 num_frames=1,
                 max_image_shape=(1080,1920)):
    ...
    def get_data(self, idx):
        ...
        if self.tri_gt is None:
            ...
            # Dilate unknow values
            uncertain = (_tri_gt > 1.0) & (_tri_gt < 254.0)
            uncertain = cv2.dilate(uncertain.astype('uint8'), cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15, 15)))
            _tri_gt[uncertain > 0] = 127
        ...
        ratio = 576 / min(fg.shape[0], fg.shape[1])
        fg = cv2.resize(fg, (int(fg.shape[1]*ratio), int(fg.shape[0]*ratio)))
        bg = cv2.resize(bg, (int(bg.shape[1]*ratio), int(bg.shape[0]*ratio)))
        a = cv2.resize(a, (int(a.shape[1]*ratio), int(a.shape[0]*ratio)))[:, :, None]
        tri_gt = cv2.resize(tri_gt, (int(tri_gt.shape[1]*ratio), int(tri_gt.shape[0]*ratio)), cv2.INTER_NEAREST)
...
class Demo_MultiInstance():
    def __init__(self, data_root):
        ...
        '''
        self.FG: [[frames]]
        self.TRI: [[only one alphas]]
        slef.seg_name: [video_name]
        '''
        seq_name = []
        FG = []
        TRI = []
        for video_name in os.listdir(os.path.join(self.data_root, 'fgr')):
            num_frames = glob.glob(os.path.join(self.data_root, 'fgr', video_name) + '/*.jpg')
            num_masks = glob.glob(os.path.join(self.data_root, 'pha', video_name) + '/*/*.png')
            num_instances = len(num_masks) // len(num_frames)
            for i_inst in range(num_instances):
                all_frames = sorted(glob.glob(os.path.join(self.data_root, 'fgr', video_name) + '/*.jpg'))
                FG.append(all_frames)
                TRI.append([all_frames[0].replace('fgr', 'pha').replace('.jpg', '') + '/%02d.png' % i_inst] * len(all_frames))
                seq_name.append(video_name + "_%02d" % i_inst)
        self.FG = FG
        self.TRI = TRI
